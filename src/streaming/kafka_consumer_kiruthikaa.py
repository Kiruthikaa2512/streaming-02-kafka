...
def process_message(row: dict[str, Any]) -> dict[str, Any]:
    """Process one consumed retail order message.

    Adds a small omnichannel retail monitoring flag inspired by
    order fulfillment and supply chain support work.
    """
    LOG.info("Processing retail omnichannel order message.")

    quantity = int(row.get("quantity", 0))
    unit_price = float(row.get("unit_price", 0))
    order_total = quantity * unit_price

    row["order_total"] = f"{order_total:.2f}"

    if quantity >= 3:
        row["fulfillment_priority"] = "review-high-quantity-order"
        LOG.info("Retail monitor alert: high quantity order detected.")
    else:
        row["fulfillment_priority"] = "standard-order"
